...
class Memory:
    def __init__(self, gamma, gae_lambda):
        ...
        # input variables of MTSR
        self.fea_mou_seq = []  # [N, tensor[sz_b, N, 8]]
        self.op_mask_seq = []  # [N, tensor[sz_b, N, 3]]
        self.fea_wor_seq = []  # [N, tensor[sz_b, M, 6]]
        self.worker_mask_seq = []  # [N, tensor[sz_b, M, M]]
        ...
        #---------
        self.worker_fatigue_time_tensor_seq = []
        self.worker_memory_time_tensor = []
        #---------
    ...
    def clear_state(self):
        del self.fea_mou_seq[:]
        del self.op_mask_seq[:]
        del self.fea_wor_seq[:]
        del self.worker_mask_seq[:]
        del self.dynamic_pair_mask_seq[:]
        del self.comp_idx_seq[:]
        del self.candidate_seq[:]
        del self.fea_pairs_seq[:]
        #------
        del self.worker_fatigue_time_tensor_seq[:]
        del self.worker_memory_time_tensor[:]
        #------
    ...
    def push(self, state):
        ...
        :return:
        """
        self.fea_mou_seq.append(state.fea_mou_tensor)
        self.op_mask_seq.append(state.op_mask_tensor)
        self.fea_wor_seq.append(state.fea_wor_tensor)
        self.worker_mask_seq.append(state.worker_mask_tensor)
        ...
        #----------
        self.worker_fatigue_time_tensor_seq.append(state.worker_fatigue_time_tensor)
        self.worker_memory_time_tensor.append(state.worker_memory_time_tensor)
    ...
    def transpose_data(self):
        ...
        """
        # 14
        t_fea_mou_seq = torch.stack(self.fea_mou_seq, dim=0).transpose(0, 1).flatten(0, 1)
        t_op_mask_seq = torch.stack(self.op_mask_seq, dim=0).transpose(0, 1).flatten(0, 1)
        t_fea_wor_seq = torch.stack(self.fea_wor_seq, dim=0).transpose(0, 1).flatten(0, 1)
        t_worker_mask_seq = torch.stack(self.worker_mask_seq, dim=0).transpose(0, 1).flatten(0, 1)
        ...
        t_worker_fatigue_time_tensor = torch.stack(self.worker_fatigue_time_tensor_seq, dim=0).transpose(0, 1).flatten(0, 1)
        t_worker_memory_time_tensor = torch.stack(self.worker_memory_time_tensor, dim=0).transpose(0, 1).flatten(0, 1)

        return t_fea_mou_seq, t_op_mask_seq, t_fea_wor_seq, t_worker_mask_seq, t_dynamicMask_seq, \
               t_Compete_m_seq, t_candidate_seq, t_pairMessage_seq, \
               t_action_seq, t_reward_seq, t_val_seq, t_done_seq, t_logprobs_seq, t_worker_fatigue_time_tensor, t_worker_memory_time_tensor
    ...
    def get_mc_advantages(self):
        reward_arr = torch.stack(self.reward_seq, dim=0)
        len_trajectory, len_envs = reward_arr.shape

        returns = torch.zeros_like(reward_arr)
        G = torch.zeros(len_envs, device=reward_arr.device)

        for i in reversed(range(len_trajectory)):
            G = reward_arr[i] + self.gamma * G
            returns[i] = G

        advantage_seq = returns - returns.mean(dim=0, keepdim=True)

        advantage_seq = (advantage_seq - advantage_seq.mean(dim=0, keepdim=True)) / \
                        (advantage_seq.std(dim=0, keepdim=True) + 1e-8)

        return advantage_seq.flatten(0, 1), None
...
class PPO:
    ...
    def update(self, memory, done_flag):
        '''
        ...
        # t_advantage_seq, v_target_seq = memory.get_mc_advantages()
        ...
        for _ in range(self.k_epochs):
            ...
            for i in range(int(num_batch)):
                ...
                pis, vals = self.policy(fea_mou=t_data[0][start_idx:end_idx],
                                        op_mask=t_data[1][start_idx:end_idx],
                                        candidate=t_data[6][start_idx:end_idx],
                                        fea_wor=t_data[2][start_idx:end_idx],
                                        worker_mask=t_data[3][start_idx:end_idx],
                                        comp_idx=t_data[5][start_idx:end_idx],
                                        dynamic_pair_mask=t_data[4][start_idx:end_idx],
                                        fea_pairs=t_data[7][start_idx:end_idx],
                                        worker_fatigue_time_tensor=t_data[13][start_idx:end_idx],
                                        worker_memory_time_tensor=t_data[14][start_idx:end_idx],
                                        done_flag=done_flag
                                        )
        ...
        return loss_epochs.item() / self.k_epochs, v_loss_epochs / self.k_epochs
