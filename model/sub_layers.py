...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.linear_or_not:
            ...
            # If linear model
            #print(f"Output shape (linear): {self.linear(x).shape}")
            ...
            for layer in range(self.num_layers - 1):
                ...
                #print(f"Shape after layer {layer}: {h.shape}")
                #print(f"Final output shape: {self.linears[self.num_layers - 1](h).shape}")
            return self.linears[self.num_layers - 1](h)
