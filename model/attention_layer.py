...
class SingleworkerAttnBlock(nn.Module):
    def __init__(self, node_input_dim, edge_input_dim, output_dim, dropout_prob):
        """
            The implementation of worker Message Attention Block
        ...
        :param dropout_prob: the parameter p for nn.Dropout()
        """
        super(SingleworkerAttnBlock, self).__init__()
    ...
    def forward(self, h, worker_mask, comp_val):
        """
        :param h: operation feature vectors with shape [sz_b, M, node_input_dim]
        :param worker_mask:  used for masking attention coefficients (with shape [sz_b, M, M])
        ...
        zero_vec = -9e15 * torch.ones_like(e)
        attention = torch.where(worker_mask > 0, e, zero_vec)
        attention = F.softmax(attention, dim=-1)
        attention = self.dropout(attention)
...
class MultiHeadworkerAttnBlock(nn.Module):
    def __init__(self, node_input_dim, edge_input_dim, output_dim, dropout_prob, num_heads, activation, concat=True):
        """
            The implementation of worker Message Attention Block with multi-head attention
        :param node_input_dim: the dimension of input node feature vectors
        :param edge_input_dim: the dimension of input edge feature vectors
        :param output_dim: the dimension of each head's output
        :param dropout_prob: the parameter p for nn.Dropout()
        ...
        :param concat: the aggregation operator, true/false means concat/averaging
        """
        super(MultiHeadworkerAttnBlock, self).__init__()
        self.dropout = nn.Dropout(p=dropout_prob)
        self.concat = concat
        self.activation = activation
        self.num_heads = num_heads

        self.attentions = [SingleworkerAttnBlock
    ...
    def forward(self, h, worker_mask, comp_val):
        ...
        h_heads = [att(h, worker_mask, comp_val) for att in self.attentions]
